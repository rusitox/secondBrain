...
- [Strands Agent Migration Review](project_strands_review.md) -- NameError crash in SSE done-event, await-after-cancel bug, thread-safety of callback, lru_cache async client (3rd!)
...
- [RD Agent MCP Review](project_rd_agent_review.md) -- Phase 6 knowledge: mcp_client.start() outside try/finally leaks on connect failure, assert for security boundary is -O-strippable
- [Knowledge Scheduler Review](project_knowledge_scheduler_review.md) -- Phase 8: bare rollback()-in-except can escape and abort whole multi-step cycle; stale "not wired" docstring
- [Backoffice Tracing Review](project_backoffice_tracing_review.md) -- Phase 0+1: usage_from_agent unguarded at call site breaks failure isolation, session isolation untested (StaticPool shared-connection risk in SQLite tests), CancelledError (15th+)
- [Phase 2 Agent Config Review](project_phase2_agentconfig_review.md) -- Config-driven agents: orchestrator ignores config.enabled (unlike domain/rd agents), filter_tools has no typo warning, tool_registry watermark-tool names untested against real code
- [Backoffice MCP Servers Review (Phase 3)](project_backoffice_mcp_phase3.md) -- MCPClient(...) construction outside try (2nd time this file), verified tool_filters ordering + encryption correctness against real strands source, SSRF/error-leak notes for Phase 4
- [Backoffice API Review (Phase 4)](project_backoffice_api_phase4.md) -- run_agent_now returns stale run data (not 409) if agent has prior history, SSRF via unvalidated McpServer.url now live, dead code pattern 3rd+ time, UNSET-sentinel PATCH verified correct end-to-end
...
- [Reconciliation Symmetric Threshold + correct_knowledge Review](project_reconciliation_symmetric_threshold_review.md) -- No criticals; confident-distinct no-op has no persisted marker so pairs renegotiate forever (cost gap); dispute_claim not scoped to entity_id
- [apply_question_answer Extraction Review](project_apply_question_answer_review.md) -- No criticals; backoffice answer-question critical bug fixed correctly; 404 conflates not-found/already-resolved/generic error (low impact, UI uses message text not status); latent same_as-detection/write-path coupling if entity deletion ever wired up
- [create_commitment Executor Review](project_create_commitment_executor_review.md) -- CRITICAL: owner="assistant" sentinel invisible to is_owned_by_user, breaks briefing + list_tasks visibility; delivered_to has no update path; clean executor/removal/prompt otherwise
